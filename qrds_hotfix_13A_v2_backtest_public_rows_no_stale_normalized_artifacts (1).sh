...
echo "[HOTFIX 13A V2] Root: $ROOT"
cd "$ROOT"

echo "[HOTFIX 13A V2] Patching normalizer cleanup..."
python - <<'PY'
from pathlib import Path

path = Path("crypto_decision_lab/src/crypto_decision_lab/reports/phase11_offline_source_normalizer_pack.py")
if not path.exists():
    raise SystemExit(f"Missing normalizer file: {path}")

text = path.read_text(encoding="utf-8")

needle = '    nd=out/"normalized"; nd.mkdir(parents=True, exist_ok=True)\n'
if needle in text and "Clear stale normalized files from prior modes/runs" not in text:
    text = text.replace(
        needle,
        needle
...
needle2 = '    norm_dir = out / "normalized"\n    norm_dir.mkdir(parents=True, exist_ok=True)\n'
if needle2 in text and "Clear stale normalized files from prior modes/runs" not in text:
    text = text.replace(
        needle2,
        needle2
        + '    # Clear stale normalized files from prior modes/runs before writing current outputs.\n'
...
if "Clear stale normalized files from prior modes/runs" not in text:
    raise SystemExit("Could not patch normalizer cleanup block. Paste the _normalize function if this repeats.")

path.write_text(text, encoding="utf-8")
print("[HOTFIX 13A V2] Normalizer cleanup patch OK.")
PY

echo "[HOTFIX 13A V2] Patching Phase 13 _discover_rows using line scanner..."
python - <<'PY'
from pathlib import Path

path = Path("crypto_decision_lab/src/crypto_decision_lab/reports/phase13_research_backtest_baseline_pack.py")
if not path.exists():
    raise SystemExit(f"Missing Phase 13 file: {path}")
...
for i, line in enumerate(lines):
    ...
    if stripped.startswith("def _discover_rows("):
        start = i
        break

if start is None:
    if "Prefer the latest normalizer manifest outputs" in text:
        print("[HOTFIX 13A V2] _discover_rows already patched.")
        raise SystemExit(0)
    context = "\n".join([ln.rstrip() for ln in lines if "_discover" in ln or "discover" in ln][:50])
    raise SystemExit("Could not locate def _discover_rows. Nearby discover lines:\n" + context)

# Preserve only top-level def/class boundaries. This function should be top-level.
end = len(lines)
...
print(f"[HOTFIX 13A V2] Replaced _discover_rows lines {start+1}-{end}.")
PY
...
echo "[HOTFIX 13A V2] Running targeted regressions..."
cd "$PROJECT"
pytest -q \
...
echo "[HOTFIX 13A V2] Running impacted tests..."
pytest -q \
...
echo "[HOTFIX 13A V2] Running full suite..."
pytest -q tests/safety tests/unit tests/integration tests/regression tests/docs

echo "[HOTFIX 13A V2] Refreshing normalizer + Phase 13 backtest without refetching..."
cd "$ROOT"
...
print("[HOTFIX 13A V2] Phase 13 summary:")
for k in [
    "gate_answer",
    "station",
    ...
    print(f"{k}: {d.get(k)}")
print("[HOTFIX 13A V2] Symbol metrics:")
...
echo "[HOTFIX 13A V2] Archiving hotfix installer if present..."
mkdir -p "$ROOT/scripts/archive/installers"
if [ -f "$ROOT/qrds_hotfix_13A_v2_backtest_public_rows_no_stale_normalized_artifacts.sh" ]; then
  mv "$ROOT/qrds_hotfix_13A_v2_backtest_public_rows_no_stale_normalized_artifacts.sh" "$ROOT/scripts/archive/installers/"
fi

echo "[HOTFIX 13A V2] Committing changes..."
cd "$ROOT"
git add -A
git commit -m "Fix Phase 13 stale normalized artifact contamination v2" || true
git push || true

echo "[HOTFIX 13A V2] Final status:"
git status --short
