...
"$ROOT_DIR/qrds_human_review.sh" "$@" | tee "$TMP_JSON"

SERVE_ENTRYPOINT="$(python - "$TMP_JSON" <<'PY'
import json, sys
payload = json.load(open(sys.argv[1], encoding='utf-8'))
print(payload.get('serve_entrypoint') or payload.get('html_path') or 'artifacts/human_review/index.html')
PY
)"
...
echo ""
echo "[QRDS 8P] Human Review / Policy Lock site ready."
echo "[QRDS 8P] Serve directory: $SERVE_DIR"
echo "[QRDS 8P] Port: $PORT"
echo ""
echo "Codespaces:"
