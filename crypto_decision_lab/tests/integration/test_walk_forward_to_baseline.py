...
def test_walk_forward_to_baseline_report(tmp_path):
    ...
    run = run_research_pipeline(
        ...
        pipeline_commit="baseline-test",
        run_id="baseline-run",
        horizons=(1, 3),
        tags=["baseline", "walk-forward", "okx"],
    )

    rows = load_research_dataset_jsonl(run["paths"]["jsonl_path"])
    target_columns = infer_baseline_target_columns(rows)
    ...
    wf_report = build_walk_forward_report(rows, splits, split_name="baseline-integration")
    ...
    assert baseline_report["schema"] == BASELINE_WALK_FORWARD_REPORT_SCHEMA_VERSION
    assert baseline_report["split_count"] == len(splits)
    assert baseline_report["aggregate"]["mean_mae"] >= 0
    assert baseline_report["operational_decision_allowed"] is False
    assert baseline_report["api_key_required"] is False
    assert baseline_report["orders_generated"] is False
    assert baseline_report["real_capital_used"] is False
    assert baseline_report["orders_allowed"] is False
    assert baseline_report["trading_signal_generated"] is False
