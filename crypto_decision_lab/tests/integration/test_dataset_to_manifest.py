...
from crypto_decision_lab.runs.manifest import build_research_run_manifest, build_research_run_manifest_report
...
def test_dataset_to_research_run_manifest_happy_path(clean_candles):
    ...
    manifest = build_research_run_manifest(
        ...
        artifacts=[{"kind": "memory_dataset", "row_count": len(dataset)}],
    )

    manifest_report = build_research_run_manifest_report(manifest)
    ...
    assert manifest_report["manifest_quality_passed"] is True
