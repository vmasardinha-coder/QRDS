...
def test_phase364_updates_current_portal(monkeypatch,tmp_path):
    ...
    output=project/"artifacts/phase364"; out=module.build(*paths,output,project_root=project,git_root=tmp_path); text=(output/"portal/index.html").read_text(encoding="utf-8"); assert out["capital_authorized_brl"]==0; assert "VOCE ESTA AQUI" in text; assert (project/"artifacts/project_portal_registry/current_portal.json").is_file()
