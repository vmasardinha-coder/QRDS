...
BASELINE_PHASE355_HEAD = "78fdf7fc45418d89f3dac0dc61beef7b8c0cce4f"
...
VALID_REMEDIATION_DECISIONS = (
    "ACCEPT_TIMESTAMP_CONSENSUS",
    "ACCEPT_DERIVATIVES_COVERAGE",
    "REJECT_ALL",
)
...
def phase_summary(phase: int, slug: str) -> Path:
    return ROOT / "docs/reports/data_remediation_v1" / f"phase{phase}_{slug}_summary.md"
...
def update_marked_block(path: Path, *, begin: str, end: str, block: str, default_title: str) -> Path:
    ...
    updated = pattern.sub(replacement, existing) if pattern.search(existing) else existing.rstrip() + "\n\n" + replacement + "\n"
