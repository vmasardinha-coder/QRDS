...
def main(argv=None):
    p=argparse.ArgumentParser(description="Build QRDS Phase 11 Offline Source Normalizer Pack.")
    p.add_argument("--output-dir",required=True); p.add_argument("--repo-root",default="")
    a=p.parse_args(argv)
    r=build_phase11_offline_source_normalizer_pack(Path(a.output_dir),a.repo_root or None)
