...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build QRDS Phase 21 Baseline Audit + Interpretable Model Benchmark Pack.")
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--repo-root", default="")
    parser.add_argument("--min-rows-per-coin", type=int, default=3500)
    args = parser.parse_args(argv)
    result = build_phase21_baseline_audit_interpretable_model_benchmark_pack(
