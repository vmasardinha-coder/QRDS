...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build QRDS Phase 26 Regime-Segmented Volatility Edge Audit Pack.")
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--repo-root", default="")
    parser.add_argument("--min-rows-per-coin", type=int, default=3500)
    args = parser.parse_args(argv)
    result = build_phase26_regime_segmented_volatility_edge_audit_pack(Path(args.output_dir), args.repo_root or None, min_rows_per_coin=args.min_rows_per_coin)
