...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build QRDS Phase 23 Volatility-First Research Benchmark Pack.")
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--repo-root", default="")
    parser.add_argument("--min-rows-per-coin", type=int, default=3500)
    args = parser.parse_args(argv)
    result = build_phase23_volatility_first_research_benchmark_pack(Path(args.output_dir), args.repo_root or None, min_rows_per_coin=args.min_rows_per_coin)
