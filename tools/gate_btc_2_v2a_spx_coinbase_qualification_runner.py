#!/usr/bin/env python3
"""Physical qualification-only capture for preregistered SPX/Coinbase V2A source."""
...
def parse_product(raw: bytes) -> dict:
    ...
    if obj.get("base_currency") != "SPX" or obj.get("quote_currency") != "USD":
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default="artifacts/gate_btc_2/v2a_spx_coinbase_qualification",
    )
    ...
    try:
        ...
        for idx in range(args.max_pages):
            ...
            params = {
                ...
                "start": rfc3339_day_start(cursor_start),
                "end": rfc3339_day_start(cursor_end + timedelta(days=1)),
            }
        ...
        if not rows:
            raise ValueError("no SPX-USD historical candles returned")
    ...
    summary = {
        "schema_version": "GATE_BTC_2_V2A_SPX_COINBASE_QUALIFICATION_V1",
        "issue": 111,
        "symbol": "SPX",
        "coin_id": "spx6900",
        "provider": PROVIDER,
        ...
        "product_identity": {
            ...
            "base_currency": product.get("base_currency") if product else None,
            "quote_currency": product.get("quote_currency") if product else None,
        },
        "pages": pages,
