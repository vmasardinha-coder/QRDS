...
def sample_pearson(xs: list[float], ys: list[float]) -> float:
    if len(xs) != len(ys) or len(xs) < 3:
        raise RuntimeError("PEARSON_INSUFFICIENT_PAIRED_OBSERVATIONS")
    ...
    if sx2 == 0 or sy2 == 0:
        raise RuntimeError("PEARSON_ZERO_VARIANCE_FAIL_CLOSED")
...
def discovery_z_params(rows: list[dict], discovery_dates: set[str]) -> dict[str, dict[str, float]]:
    ...
    for component in (IPCA, SELIC):
        ...
        if sd == 0:
            raise RuntimeError(f"ZERO_VARIANCE_{component}_FAIL_CLOSED")
...
def scalar_feature(row: dict, case: dict, params: dict[str, dict[str, float]]) -> float:
    zi = (float(row[IPCA]) - params[IPCA]["mean"]) / params[IPCA]["sample_sd"]
    zs = (float(row[SELIC]) - params[SELIC]["mean"]) / params[SELIC]["sample_sd"]
    return float(case["weights"]["IPCA"]) * zi + float(case["weights"]["SELIC"]) * zs
...
def target_returns_for_dates(target_dates: list[str], yearly_levels: dict[int, dict[str, float]]) -> dict[str, float]:
    ...
    for d in target_dates:
        ...
        # Ensure previous close is an actual prior B3 observation, not an imputed calendar day.
...
def execute(features: dict, partitions: dict, cases_auth: dict, stat_auth: dict, route: dict,
    ...
    # Crucial blindness boundary: before each case validates, only 2024/2025 are transported.
    pre_holdout_levels = {2024: fetch_year(2024, route), 2025: fetch_year(2025, route)}
    pre_dates = discovery_dates + validation_dates
    pre_returns = target_returns_for_dates(pre_dates, pre_holdout_levels)
    ...
    for case in cases:
        ...
        result = {
            ...
            "discovery": {"pearson_r": rd, "n": nd},
            "validation": {"pearson_r": rv, "n": nv, "same_sign": same_sign(rd, rv), "magnitude_floor": 0.50 * abs(rd), "pass": validation_pass},
            "holdout": {"started": False, "target_year_2026_fetched_for_this_case": False, "pearson_r": None, "n": 0, "pass": None},
            "terminal_status": "REJECTED_VALIDATION_NO_RETUNE",
        }
        if validation_pass:
            # Fetch 2026 only now, for this already-validated case. Do not retain raw provider bytes.
            ...
            result["holdout"] = {
                "started": True,
                "target_year_2026_fetched_for_this_case": True,
                "pearson_r": rh,
                "n": nh,
                "same_sign": same_sign(rd, rh),
                "magnitude_floor": 0.50 * abs(rd),
                "pass": hp,
            }
            result["terminal_status"] = "SURVIVOR_TO_SEPARATE_BLINDED_PROSPECTIVE_ZERO_RETROACTIVE_CREDIT" if hp else "REJECTED_HOLDOUT_NO_RETUNE"
...
def main() -> int:
    ...
    print(json.dumps({"status": result["status"], "cases": [{"case_id": x["case_id"], "validation_pass": x["validation"]["pass"], "holdout_started": x["holdout"]["started"], "terminal_status": x["terminal_status"]} for x in result["cases"]], "survivors": result["survivors"]}, sort_keys=True))
    return 0
