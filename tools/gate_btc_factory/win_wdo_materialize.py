...
def expiry_key(ticker,day):
 ...
 y=2000+int(m.group(3)); mo=MONTH[m.group(2)]; d=date.fromisoformat(day)
 delta=(y-d.year)*12+mo-d.month
 return (delta if delta>=0 else 10000+abs(delta),ticker)
def qualified_rows(start,end):
 raw=[]; manifest=[]; missing=[]; d=start
 while d<=end:
  if d.weekday()<5:
   token=d.strftime('%y%m%d'); url=f'https://www.b3.com.br/pesquisapregao/download?filelist=PR{token}.zip'
   ...
   elif d.isoformat()!=GAP and status==200:
    raise SystemExit(f'FAIL_CLOSED: published PriceReport lacks qualified WIN/WDO rows on {d}')
...
def select_series(raw):
 ...
 for day in dates:
  for root in ('WIN','WDO'):
   ...
   if prior:
    # frozen rule: greatest t liquidity; tie = nearest expiry. Selection applies only on t+1.
    prev[root]=sorted(prior,key=lambda x:(-x['liquidity'],expiry_key(x['ticker'],day)))[0]['ticker']
 m={}
 ...
 for i,r0 in enumerate(part):
  ...
  for n in (1,5,20,60):
   for root in ('WIN','WDO'):
    ...
    window=part[i:i+n+1]; tick=r0[root+'_ticker']
    r[key]=(window[-1][root+'_close']/r0[root+'_close']-1) if all(x[root+'_ticker']==tick for x in window) else None
  out.append(r)
 return out

def partition(eligible):
 n=len(eligible); a1=n//2; a2=a1+(n*3//10)
 rawparts={'discovery':eligible[:a1],'validation':eligible[a1:a2],'holdout':eligible[a2:]}
 # Frozen preregistration: 60 eligible-session embargo at each later partition boundary.
 parts={'discovery':rawparts['discovery'],'validation':rawparts['validation'][EMBARGO:],'holdout':rawparts['holdout'][EMBARGO:]}
 return {k:add_targets(v) for k,v in parts.items()}
