...
SAMPLE_DATES = ("2025-01-02", "2025-06-02", "2025-12-12")
GITHUB_QUERIES = (
    ...
    for query in GITHUB_QUERIES:
        try:
            ...
            for item in r.json().get("items", []):
                ...
                if api_url:
                    try:
                        fr = session.get(str(api_url), headers=headers, timeout=20)
                        fr.raise_for_status()
                        obj = fr.json()
            ...
            out.append({"query": query, "status": "SEARCH_ERROR_FAIL_CLOSED", "error": f"{type(exc).__name__}: {exc}"})
    return out
...
def search(root_cause: dict[str, Any], gate_path: Path, runtime_root: Path, token: str | None = None, session: requests.Session | None = None) -> dict[str, Any]:
    ...
    s.headers.update({"User-Agent": "QRDS-invalidated-source-qualification-search/1.0"})
    gate = validate_existing_gate(gate_path, runtime_root)
    official = [_sample_response(s, B3_LEGACY.format(date=d)) for d in SAMPLE_DATES]
    official.append(_sample_response(s, B3_BDI_ROOT))
