...
def _sample_response(session: requests.Session, url: str) -> dict[str, Any]:
    try:
        with session.get(url, timeout=30, stream=True, allow_redirects=True, headers={"Range": "bytes=0-65535"}) as r:
            ...
            return {
                "url": url, "final_url": str(r.url), "http_status": status,
                "reachable": 200 <= status < 400, "content_type": r.headers.get("content-type"),
                "content_length": r.headers.get("content-length"), "sample_bytes": len(sample),
    ...
    for query in GITHUB_QUERIES:
        try:
            ...
            for item in r.json().get("items", []):
                ...
                discovered.append({"query": query, "repository": repo, "path": path,
...
def _find_key(obj: Any, wanted: str) -> Any:
    if isinstance(obj, dict):
        if wanted in obj: return obj[wanted]
...
def _source_contract(root_cause: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "bar_interval": "M5",
        "minimum_sessions_required": _find_key(root_cause, "minimum_sessions_required"),
    ...
    try:
        ...
        for record in packet.get("records") or []:
            ...
            if _win_identity_hint(identity_text):
                matches.append({"symbol": record.get("symbol"), "description": record.get("description"),
                                "path": record.get("path"), "latest_observation_utc": record.get("latest_observation_utc"),
                                "captured_bar_count": len(record.get("bars") or [])})
        return {
            "status": "MT5_WIN_IDENTITY_SURFACE_DISCOVERED_REVIEW_REQUIRED" if matches else "MT5_PACKET_NO_WIN_IDENTITY_MATCH",
            "source_packet_sha256": packet.get("packet_sha256"), "source_identity_semantics": packet.get("source_identity_semantics"),
            "matching_record_count": len(matches), "matching_records": matches[:20],
            "source_admission_pass": False, "prospective_credit": 0, "historical_backfill_credit": 0,
        ...
        return {"status": "MT5_PACKET_PARSE_ERROR_FAIL_CLOSED", "error": f"{type(exc).__name__}: {exc}",
                "source_admission_pass": False, "blocks_other_sources": False}
...
def search(root_cause: dict[str, Any], gate_path: Path, runtime_root: Path, token: str | None = None,
    ...
    s.headers.update({"User-Agent": "QRDS-invalidated-source-qualification-search/3.0"})
    ...
    return {
        "schema": SCHEMA, "generated_at_utc": utcnow(), "status": status, "cadence_hours": CADENCE_HOURS,
        "root_cause_classification": root_cause.get("root_cause_classification"),
        "affected_family_count": int(affected.get("family_count") or 0), "source_gate": gate,
        "frozen_source_contract": _source_contract(root_cause), "blocking_requirements": blockers,
        "search_strategy": {
        ...
        "search": {
            "official_b3_probes": official, "mt5_read_only_source_evidence": mt5,
            "independent_free_github_candidates": github, "reachable_official_probe_count": reachable_official,
            "schema_identity_candidate_count": promising_free, "irrelevant_identity_candidate_count": irrelevant,
        ...
        "qualification_policy": {
            "free_or_official_auditable_required": True, "publication_semantics_required": True,
        ...
        "safety": {
            "research_only": True, "shadow_only": True, "not_approved": True, "engine_feed": False, "orders": 0,
...
def main() -> int:
    ...
    out = search(root, Path(args.gate), Path(args.runtime_root), token=os.environ.get("GITHUB_TOKEN"),
                 mt5_packet_path=Path(args.mt5_packet) if args.mt5_packet else None)
    p = Path(args.output)
    p.parent.mkdir(parents=True, exist_ok=True)
    p.write_text(json.dumps(out, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(json.dumps({"status": out["status"], "affected_family_count": out["affected_family_count"],
                      "source_gate": out["source_gate"]["reason"], "blocking_requirements": out["blocking_requirements"]}))
    return 0
