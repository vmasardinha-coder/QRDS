...
def qualify(intake, protocol, evidence_dir):
    ...
    for f in intake.get("families",[]):
        ...
        if f.get("status") == "BLOCKED_PREREG_SEMANTICS_UNDECIDABLE" or f.get("semantic_preregistration_complete") is False:
            base.update(
                status="BLOCKED_PREREG_SEMANTICS_UNDECIDABLE",
                blocker_reason="SOURCE_COST_PROTOCOL_REQUIRES_ALREADY_FROZEN_FEATURE_WINDOW_LOOKBACK_TARGET",
                next_stage="SEPARATE_SEMANTIC_PREREGISTRATION_REQUIRED_BEFORE_SOURCE_COST",
            )
        ...
        rows.append(base)
    return {
        "schema":"qrds.factory.grammar_source_cost_qualification_runtime.v1",
        "protocol_schema":protocol["schema"],
        "families":rows,
        "qualified_count":sum(x["status"]=="SOURCE_COST_QUALIFIED" for x in rows),
        "waiting_count":sum(x["status"]=="WAITING_SOURCE_COST_EVIDENCE" for x in rows),
        "blocked_semantics_count":sum(x["status"]=="BLOCKED_PREREG_SEMANTICS_UNDECIDABLE" for x in rows),
        "rejected_count":sum(x["status"]=="REJECT_SOURCE_COST_FAIL_CLOSED" for x in rows),
