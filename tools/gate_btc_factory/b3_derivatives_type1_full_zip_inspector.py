#!/usr/bin/env python3
"""Evidence-only full ZIP inspector for the official B3 type=1 DRV surface.

Downloads one recent official response completely, hashes the exact ZIP bytes, and
inspects member names/content for exact WIN identity and tabular trade schema.
It never admits the source, creates SOURCE_GATE, grants credit, or reads economics.
"""
...
SCHEMA = "qrds.factory.b3_derivatives_type1_full_zip_inspector.v1"
...
def inspect(session: requests.Session, probe_date: str) -> dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory() as td:
            ...
            out["drv_filename_observed"] = "_DRV" in str(out.get("content_disposition") or "") or any("_DRV" in str(m.get("name") or "") for m in out["members"])
            out["status"] = (
                "WIN_IDENTITY_PHYSICALLY_OBSERVED_NEEDS_SEMANTIC_QUALIFICATION"
                if symbols else "TYPE1_DRV_ZIP_HAS_NO_WIN_IDENTITY_FAIL_CLOSED"
            )
            out["next_requirement"] = (
                "PROVE_SCHEMA_TIMEZONE_PUBLICATION_REVISION_PIT_AND_161_SESSION_COVERAGE_WITHOUT_BACKFILL_CREDIT"
                if symbols else "DO_NOT_ADMIT_TYPE1_AS_WIN_SOURCE_CONTINUE_OFFICIAL_SOURCE_DISCOVERY"
            )
