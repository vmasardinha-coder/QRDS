...
def materialize(packet_path):
 ...
 return {'schema':'qrds.factory.xawinwdo_regime_005.materialized.v1','family_id':FAMILY,'source':'MT5_TERMINAL_AUTHORIZED_PRIMARY_EXPERIMENT_XAWINWDO_ONLY','source_packet_sha256':sha(packet_path),'timezone':'America/Sao_Paulo','bar_granularity':'M5_PHYSICAL_AGGREGATED_TO_SESSION_CLOSE','roll_rule':'t_minus_1_liquidity','return_rule':'log_close_to_close_no_synthetic_returns_across_rolls','known_gap_policy':'drop_only','split_rule':'50_30_20_chronological','embargo_sessions':20,'aligned_session_count':len(rows),'usable_return_count':len(usable),'splits':sp,'split_counts':{k:len(v) for k,v in sp.items()},'safety':{'RESEARCH_ONLY':True,'SHADOW_ONLY':True,'NOT_APPROVED':True,'ENGINE_FEED':False,'ORDERS':0,'REAL_CAPITAL':0,'NO_RETUNE':True,'NO_BACKFILL':True,'NO_COUNTER_RESET':True,'FAIL_CLOSED':True},'next_stage':'DISCOVERY_ONLY_VALIDATION_HOLDOUT_SEALED' if all(len(sp[k])>0 for k in ('discovery','validation','holdout')) else 'FAIL_CLOSED_INSUFFICIENT_PHYSICAL_OVERLAP'}
def main():
