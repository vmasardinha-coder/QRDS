...
def main():
    ...
    try:
        ...
        result={"schema":"qrds.factory.invalidated_512.mt5_strict_v2_source_result.v2","authority_issue":693,"evaluation_namespace":NAMESPACE,"status":"MT5_SECONDARY_CROSS_VALIDATION_QUALIFIED" if qualified_for_cross_validation else "MT5_SECONDARY_CROSS_VALIDATION_FAIL_CLOSED","window":{"discovery":{"start":START,"end":DISC_END},"replication":{"start":REPL_START,"end":END}},"raw_capture_sha256":rh,"normalized_candidate_sha256":ch,"enumerated_exact_win_contract_count":len(metas),"raw_bar_count":sum(len(v) for v in raw.values()),"candidate_bar_count":len(cand),"valid_session_counts":c,"minimum_required":{"total":MIN_TOTAL,"discovery":MIN_PART,"replication":MIN_PART},"source_gates":gates,"capture_errors":errors,"source_role":SOURCE_ROLE,"usage_constraint":USAGE_CONSTRAINT,"cross_validation_qualification_pass":qualified_for_cross_validation,"source_admission_pass":False,"may_be_primary_source":False,"may_reconstruct_lost_clocks":False,"requalification_economics_allowed":False,"scientific_family_credit":0,"prospective_credit":0,"historical_backfill_credit":0,"safety":SAFETY}; result["result_sha256"]=digest(result)
