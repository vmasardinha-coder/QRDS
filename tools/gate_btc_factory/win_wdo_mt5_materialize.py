...
def aggregate(packet):
 ...
 for r in packet.get('records',[]):
  ...
  root=sym[:3]
  for b in r.get('bars',[]):
   ts=datetime.fromisoformat(b['timestamp_utc'].replace('Z','+00:00')).astimezone(TZ)
   d=ts.date().isoformat(); x=daily[root][(d,sym)]
...
def select_root(daily,root):
 ...
 for i,d in enumerate(days):
  if i==0: continue
  prev=days[i-1]
  eligible=[(x['volume'],sym) for sym,x in byday[prev].items() if sym in byday[d]]
  if not eligible: continue
  _,sym=max(eligible,key=lambda z:(z[0],z[1]))
  out.append({'date':d,'symbol':sym,'close':byday[d][sym]['close'],'selection_from_session':prev})
 return out

def split(rows):
 n=len(rows); d=int(n*.50); v=int(n*.30); h=n-d-v
 raw={'discovery':rows[:d],'validation':rows[d:d+v],'holdout':rows[d+v:]}
 out={}
 for k,part in raw.items():
  lo=EMBARGO if k!='discovery' else 0; hi=len(part)-EMBARGO if k!='holdout' else len(part)
  out[k]=part[lo:max(lo,hi)]
 return out
...
def materialize(packet_path):
 ...
 assert s.get('MT5_READ_ONLY') is True and s.get('NO_ORDER_SEND') is True
 assert s.get('ORDERS')==0 and s.get('REAL_CAPITAL')==0 and s.get('ENGINE_FEED') is False
 assert s.get('NO_RETUNE') is True and s.get('NO_BACKFILL') is True
 daily=aggregate(p); win=select_root(daily,'WIN'); wdo=select_root(daily,'WDO')
 wi={x['date']:x for x in win}; wd={x['date']:x for x in wdo}; dates=sorted(set(wi)&set(wd))
 rows=[]; prev=None
 for d in dates:
  ...
  if prev:
   same=a['symbol']==prev['WIN_contract'] and b['symbol']==prev['WDO_contract']
   row['roll_boundary']=not same
   if same:
    row['WIN_return']=logret(prev['WIN_close'],a['close']); row['WDO_return']=logret(prev['WDO_close'],b['close'])
  rows.append(row); prev=row
 usable=[r for r in rows if r['WIN_return'] is not None and r['WDO_return'] is not None]
 sp=split(usable)
...
def main():
 ...
 out=materialize(Path(a.packet)); q=Path(a.out); q.parent.mkdir(parents=True,exist_ok=True); q.write_text(json.dumps(out,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
 print(json.dumps({'family':FAMILY,'aligned':out['aligned_session_count'],'usable':out['usable_return_count'],'split_counts':out['split_counts'],'next_stage':out['next_stage']},sort_keys=True))
