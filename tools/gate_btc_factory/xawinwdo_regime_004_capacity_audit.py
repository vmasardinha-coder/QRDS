...
def main():
 ...
 n=len(best); d=int(n*.5); v=int(n*.3)
 counts={"discovery":max(0,d-EMBARGO),"validation":max(0,(d+v-EMBARGO)-(d+EMBARGO)),"holdout":max(0,n-(d+v+EMBARGO))}
 capacity=n>MAX_LOOKBACK and all(x>0 for x in counts.values())
 out={"schema":"qrds.factory.xawinwdo_regime_004.capacity_audit.v1","family_id":FAMILY,"economics_read":False,"selection_rule":"largest_contiguous_synchronized_physical_session_block_earliest_tie","synchronized_session_count_all":len(ds),"block_count":len(bs),"largest_block":{"start":best[0] if best else None,"end":best[-1] if best else None,"sessions":n},"frozen_rolling_dependence_windows":[20,60],"frozen_max_lookback_sessions":MAX_LOOKBACK,"embargo_sessions":EMBARGO,"split_rule":"50_30_20_chronological","post_embargo_split_capacity":counts,"capacity_pass":capacity,"next_stage":"DISCOVERY_ALLOWED" if capacity else "FAIL_CLOSED_INSUFFICIENT_CONTIGUOUS_PHYSICAL_CAPACITY","safety":SAFETY}
