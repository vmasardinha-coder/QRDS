...
# Legacy official B3/BM&FBovespa historical-series route. Capture is an access
# qualification only. Bytes are never parsed into target returns at this stage.
...
def parse_bcb(rec: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {"json_valid": True, "row_count": len(rows), "fields": fields}
    except Exception as exc:  # noqa: BLE001
...
def main() -> int:
    ...
    bcb_meta = get(BCB_METADATA)
    bcb_ipca = get(bcb_query("IPCA"))
    bcb_selic = get(bcb_query("Selic"))
    bcb_ipca_shape = parse_bcb(bcb_ipca)
    bcb_selic_shape = parse_bcb(bcb_selic)
    ...
    bcb_capture_pass = (
        bcb_meta.get("reachable") is True
        and bcb_ipca_shape["json_valid"] and bcb_ipca_shape["row_count"] > 0
        and bcb_selic_shape["json_valid"] and bcb_selic_shape["row_count"] > 0
        and bcb_required.issubset(bcb_fields)
    )

    cvm_inf_meta = get(CVM_INF_META)
    cvm_inf = get(CVM_INF_SAMPLE)
    cvm_delivery_meta = get(CVM_DELIVERY_META)
    cvm_delivery = get(CVM_DELIVERY_SAMPLE)
    cvm_class_meta = get(CVM_CLASS_META)
    cvm_class = get(CVM_CLASS_REGISTRY)
    inf_schema = zip_schema(cvm_inf)
    delivery_schema = zip_schema(cvm_delivery)
    class_schema = zip_schema(cvm_class)
    class_meta_schema = zip_schema(cvm_class_meta)
    ...
    cvm_capture_pass = (
        cvm_inf_meta.get("reachable") is True
        and cvm_delivery_meta.get("reachable") is True
        and inf_schema.get("zip_valid") is True
        and delivery_schema.get("zip_valid") is True
        and class_schema.get("zip_valid") is True
        and class_meta_schema.get("zip_valid") is True
        and all(inf_tokens["present"].values())
    )

    b3_attempts = [get(url) for url in B3_COTAHIST_CANDIDATES]
    # Deliberately do not inspect/parse COTAHIST bytes even if reachable.
    b3_reachable = any(x.get("reachable") and x.get("byte_count", 0) > 0 for x in b3_attempts)

    source_gate = {
        "BCB_OLINDA_EXPECTATIVASMERCADOANUAIS": {
            "physical_capture_pass": bcb_capture_pass,
            "schema_pass": bcb_required.issubset(bcb_fields),
            "required_fields": sorted(bcb_required),
            "observed_fields": sorted(bcb_fields),
            "metadata": public(bcb_meta),
            "ipca_sample": {**public(bcb_ipca), **bcb_ipca_shape},
            "selic_sample": {**public(bcb_selic), **bcb_selic_shape},
            "official_publication_cadence_documented": True,
            "exact_historical_intraday_publication_timestamp_proven": False,
            "pit_admission_pass": False,
            "status": "PHYSICAL_CAPTURE_PASS_PIT_TIMESTAMP_STILL_UNPROVEN" if bcb_capture_pass else "PHYSICAL_CAPTURE_FAIL",
        },
        "CVM_FI_INF_DIARIO_PLUS_DELIVERY_AND_CLASS_REGISTRY": {
            "physical_capture_pass": cvm_capture_pass,
            "inf_diario_meta": public(cvm_inf_meta),
            "inf_diario_meta_tokens": inf_tokens,
            "inf_diario_sample": {**public(cvm_inf), **inf_schema},
            "delivery_meta": public(cvm_delivery_meta),
            "delivery_meta_tokens": delivery_tokens,
            "delivery_sample": {**public(cvm_delivery), **delivery_schema},
            "class_registry_meta": {**public(cvm_class_meta), **class_meta_schema},
            "class_registry_sample": {**public(cvm_class), **class_schema},
            "revision_semantics_documented": True,
            "availability_mapping_requires_delivery_join": True,
            "pit_admission_pass": False,
            "status": "PHYSICAL_CAPTURE_PASS_PIT_JOIN_NOT_YET_MATERIALIZED" if cvm_capture_pass else "PHYSICAL_CAPTURE_FAIL",
        },
        "B3_COTAHIST_OFFICIAL": {
            "physical_capture_pass": b3_reachable,
            "attempts": [public(x) for x in b3_attempts],
            "target_bytes_parsed": False,
            "target_outcomes_read": False,
            "pit_admission_pass": False,
            "status": "TARGET_PHYSICAL_CAPTURE_REACHABLE_UNPARSED" if b3_reachable else "TARGET_CAPTURE_FAIL",
        },
    }
    ...
    result = {
        ...
        "stage": "PHYSICAL_SOURCE_CAPTURE_ONLY",
        "status": "FEATURE_SOURCES_CAPTURED_TARGET_PENDING" if bcb_capture_pass and cvm_capture_pass and not b3_reachable else (
            "PHYSICAL_SOURCES_CAPTURED_NO_OUTCOME_READ" if bcb_capture_pass and cvm_capture_pass and b3_reachable else "SOURCE_CAPTURE_FAIL_CLOSED"
        ),
        "sources": source_gate,
        "next_gate": "PIT_SEMANTICS_AND_TARGET_SOURCE_QUALIFICATION",
    ...
    # BCB/CVM are required feature sources for this stage. B3 is allowed to
    # remain an explicit target capture fail without turning infra red.
