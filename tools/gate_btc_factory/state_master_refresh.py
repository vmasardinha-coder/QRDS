...
def _canonical_preregs(root: Path) -> list[tuple[int, int, Path]]:
...
def sync_b3_frontier(src: dict, root: Path = ROOT) -> dict:
    """Refresh only the materialized B3 generation pointer from canonical repo files.

    This never invents a family, changes parameters, reads partial economics, or
    promotes a survivor. Only a terminal CLOSED_NO_* result with the immutable
    safety boundary may clear obsolete issue/PR pointers. A newer prereg file may
    mark the already-materialized next generation as active/source-QA-ready.
    """
    ...
    if preregs:
        pstart, pend, prereg_path = preregs[-1]
