...
{'channel_id':'BCB_FOCUS_EXPECTATIONS_REVISION_B3_TRANSMISSION','queries':['Brazil Focus survey expectations stock market','Brazil central bank expectations Ibovespa'],'mechanism':'Published Brazilian central-bank market-expectation revisions may transmit macro expectation information to strictly subsequent B3 returns.','required_new_data':['BCB Focus expectations statistics','official publication timing','B3 market data','exchange calendar'],'official_free_source_candidates':['BCB','B3']},
{'channel_id':'B3_INDEX_REBALANCE_ANNOUNCEMENT_TRANSMISSION','queries':['Brazil Ibovespa index rebalancing announcement price impact','B3 index reconstitution Brazil'],'mechanism':'Published B3 index preview and rebalance announcements may create predictable mechanical-flow information before later implementation dates.','required_new_data':['B3 index preview/final portfolio files','announcement/effective dates','B3 security prices','exchange calendar'],'official_free_source_candidates':['B3']},
{'channel_id':'BCB_FX_SWAP_AUCTION_B3_TRANSMISSION','queries':['Brazil central bank FX swap auction exchange rate','Brazilian central bank currency swap auction'],'mechanism':'Preannounced Brazilian central-bank FX-swap auctions and result communications define public auction events that may transmit policy and currency information to subsequent BRL and B3 observations.','required_new_data':['BCB FX-swap auction announcements/results','official event timing','BRL market data','B3 market data','calendars'],'official_free_source_candidates':['BCB','B3']},
{'channel_id':'CVM_FUND_FLOW_B3_TRANSMISSION','queries':['Brazil mutual fund flows equity returns','Brazil investment fund redemptions stock market'],'mechanism':'Lagged Brazilian investment-fund subscriptions and redemptions, observed only after official publication, may transmit institutional flow pressure to subsequent B3 returns.','required_new_data':['CVM daily fund reports','fund classification','official publication timing','B3 prices','exchange calendar'],'official_free_source_candidates':['CVM','B3']},
{'channel_id':'B3_FOREIGN_INVESTOR_FLOW_TRANSMISSION','queries':['Brazil foreign investor flows stock market','Brazilian foreign capital flows Ibovespa'],'mechanism':'Officially published lagged foreign-investor movement aggregates may transmit capital-flow pressure to strictly later B3 observations.','required_new_data':['B3 foreign investor movement data','official publication timing','B3 index/security prices','exchange calendar'],'official_free_source_candidates':['B3']},
]
