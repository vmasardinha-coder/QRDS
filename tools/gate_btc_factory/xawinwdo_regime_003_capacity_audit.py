...
FAMILY="XAWINWDO_REGIME_003"; PAT=re.compile(r"^(WIN|WDO)[FGHJKMNQUVXZ]\d{2}$"); TZ=ZoneInfo("America/Sao_Paulo"); EMBARGO=60; MAX_LOOKBACK=252
...
def main():
 ...
 out={"schema":"qrds.factory.xawinwdo_regime_003.capacity_audit.v1","family_id":FAMILY,"economics_read":False,"selection_rule":"largest_contiguous_synchronized_physical_session_block_earliest_tie","synchronized_session_count_all":len(ds),"block_count":len(bs),"largest_block":{"start":best[0] if best else None,"end":best[-1] if best else None,"sessions":n},"frozen_max_lookback_sessions":MAX_LOOKBACK,"embargo_sessions":EMBARGO,"post_embargo_split_capacity":counts,"capacity_pass":capacity,"next_stage":"DISCOVERY_ALLOWED_AFTER_CANDIDATE_PIPELINE_IMPLEMENTED" if capacity else "FAIL_CLOSED_INSUFFICIENT_CONTIGUOUS_PHYSICAL_CAPACITY","safety":SAFETY}
