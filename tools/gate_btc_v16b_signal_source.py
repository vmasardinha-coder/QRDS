...
Builds deterministic CMC->Binance Spot/USDT mappings and causal daily inputs for
CURRENT_PROSPECTIVE_PIT rows. No aliases, no string-appended USDT, no ranking-
conditioned repair, no backfill. RESEARCH_ONLY / SHADOW_ONLY / NOT_APPROVED.
"""
...
PRODUCER_VERSION = "V16B_SIGNAL_SOURCE_V1_20260909"
...
def build_mapping(cmc: pd.DataFrame, exchange_info: dict[str, Any], signal_date: pd.Timestamp) -> pd.DataFrame:
    ...
    for r in cmc.itertuples(index=False):
        ...
        if cmc_counts.get(r.symbol, 0) != 1:
            ...
            m = matches[0]
            market = str(m["symbol"]).upper()
            base = str(m["baseAsset"]).upper()
            quote = str(m["quoteAsset"]).upper()
        rows.append({
            "signal_date": signal_date.date().isoformat(),
            "cmc_id": int(r.id), "cmc_symbol": r.symbol, "cmc_slug": r.slug, "cmc_rank": int(r.cmc_rank),
            "market_symbol": market, "base_asset": base, "quote_asset": quote,
...
def normalize_klines(symbol: str, rows: list[list[Any]], signal_date: pd.Timestamp) -> pd.DataFrame:
    ...
    for row in rows:
        ...
        if close_ms > cutoff_ms:
            continue
        out.append({
            "date": pd.to_datetime(open_ms, unit="ms", utc=True).date().isoformat(),
            "symbol": symbol.upper(),
            "close": float(close),
            "volume_usd": float(qvol),
        })
...
def build_live(snapshot: Path, evidence_path: Path, signal_date: pd.Timestamp, out_dir: Path, contract_path: Path = CONTRACT_PATH) -> dict[str, Any]:
    ...
    server = _get_json(sess, f"{BASE_URL}/api/v3/time")
    retrieved["server_time"] = datetime.now(timezone.utc).isoformat()
    ...
    exchange = _get_json(sess, f"{BASE_URL}/api/v3/exchangeInfo")
    retrieved["exchange_info"] = datetime.now(timezone.utc).isoformat()
    exchange_path = raw_dir / "binance_exchange_info.json"; _write_json(exchange_path, exchange)

    cmc = load_cmc_assets(snapshot)
    mapping = build_mapping(cmc, exchange, signal_date)
    mapping_path = out_dir / "mapping.csv"
    mapping.to_csv(mapping_path, index=False, lineterminator="\n")
    mapped = sorted(set(mapping.loc[mapping.mapping_status.eq("MAPPED"), "market_symbol"]))
    if "BTCUSDT" not in mapped:
        mapped.append("BTCUSDT")
    mapped = sorted(set(mapped))

    start_ms = int((cutoff - pd.Timedelta(days=900)).timestamp() * 1000)
    end_ms = int(cutoff.timestamp() * 1000) - 1
    daily_parts: list[pd.DataFrame] = []
    raw_hashes: dict[str, str] = {}
    for symbol in mapped:
        rows = _get_json(sess, f"{BASE_URL}/api/v3/klines", {"symbol": symbol, "interval": "1d", "startTime": start_ms, "endTime": end_ms, "limit": 1000})
        retrieved[f"kline:{symbol}"] = datetime.now(timezone.utc).isoformat()
    ...
    u = mapping[mapping.mapping_status.eq("MAPPED")].copy()
    u = pd.DataFrame({
        "signal_date": signal_date.date().isoformat(),
        "symbol": u["market_symbol"],
        "evidence_class": "CURRENT_PROSPECTIVE_PIT",
        "snapshot_effective_date": signal_date.date().isoformat(),
        "retrieved_at_utc": str(cmc_ev["available_at_utc"]),
        "source_ref": str(cmc_ev["source_ref"]),
        "snapshot_sha256": sha256_file(snapshot),
    }).sort_values("symbol")
    universe_path = out_dir / "weekly_universe.csv"; u.to_csv(universe_path, index=False, lineterminator="\n")

    manifest = {
        "schema": "gate_btc.v16b.signal_market_source_manifest.v1",
        "producer_version": PRODUCER_VERSION,
        "signal_date": signal_date.date().isoformat(),
        "contract_id": contract["contract_id"],
        "contract_sha256": sha256_file(contract_path),
        "cmc_snapshot_sha256": sha256_file(snapshot),
        "cmc_evidence_sha256": sha256_file(evidence_path),
        "exchange_info_sha256": sha256_file(exchange_path),
        "server_time_sha256": sha256_file(server_path),
        "mapping_sha256": sha256_file(mapping_path),
        "daily_prices_sha256": sha256_file(daily_path),
        "weekly_universe_sha256": sha256_file(universe_path),
        "raw_kline_sha256_by_symbol": raw_hashes,
        "retrieved_at_utc_by_source": retrieved,
        "mapped_count": int((mapping.mapping_status == "MAPPED").sum()),
        "unmapped_count": int((mapping.mapping_status == "UNMAPPED").sum()),
        "benchmark": "BTCUSDT",
        "market_venue": "BINANCE_SPOT_USDT",
        "volume_usd_semantics": "BINANCE_KLINE_QUOTE_ASSET_VOLUME_USDT_NO_FX_CONVERSION",
        "NO_BACKFILL": True, "NO_LATE_SEAL": True, "NO_RETUNE": True,
        "RESEARCH_ONLY": True, "SHADOW_ONLY": True, "NOT_APPROVED": True,
        "ENGINE_FEED": False, "ORDERS": 0, "REAL_CAPITAL": 0, "FAIL_CLOSED": True,
    }
...
def main() -> int:
    ...
    p.add_argument("--cmc-snapshot", required=True)
    p.add_argument("--cmc-evidence", required=True)
    p.add_argument("--signal-date", required=True)
    p.add_argument("--out-dir", required=True)
    p.add_argument("--contract", default=str(CONTRACT_PATH))
    a = p.parse_args()
    manifest = build_live(Path(a.cmc_snapshot), Path(a.cmc_evidence), pd.Timestamp(a.signal_date), Path(a.out_dir), Path(a.contract))
    print(json.dumps({"status": "OK", "mapped": manifest["mapped_count"], "unmapped": manifest["unmapped_count"], "ORDERS": 0, "REAL_CAPITAL": 0}, sort_keys=True))
    return 0
