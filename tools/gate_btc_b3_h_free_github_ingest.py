...
from tools.gate_btc_b3_h_win_contract_manifest import ALLOWED_START, H1_CUTOFF, win_front
...
def parse_profit_csv(text: str) -> pd.DataFrame:
    ...
    out['timestamp'] = pd.to_datetime(df[cols['data']].str.strip() + ' ' + df[cols['hora']].str.strip(), dayfirst=True, errors='raise')
...
def build(out_csv: Path, out_meta: Path, out_report: Path) -> dict:
    ...
    # sanity: explicit contracts only and WIN 5-point grid
    ...
    meta={
        ...
        'h1_cutoff_exclusive':H1_CUTOFF.isoformat(),'h1_economics_read':False,
        'orders':0,'real_capital':0,
        'sha256':hashlib.sha256(raw).hexdigest(),
    }
    out_meta.write_text(json.dumps(meta,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
    report={
        'status':'PASS_SOURCE_INGEST_ONLY','rows':len(x),
        'sessions':pd.to_datetime(x['timestamp']).dt.date.nunique(),
        'first_timestamp':x['timestamp'].iloc[0],'last_timestamp':x['timestamp'].iloc[-1],
        'symbols':sorted(x['symbol'].unique().tolist()),'files':file_report,
