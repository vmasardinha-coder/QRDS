...
candles, with zero credit/admission and no route switching. The frozen provider
is BINANCE_SPOT; transport uses Binance's public data host because api.binance.com
is geoblocked from GitHub-hosted runners.
"""
...
BINANCE_PUBLIC_DATA="https://data-api.binance.vision"
...
def main()->int:
    ...
    exraw=req(BINANCE_PUBLIC_DATA+"/api/v3/exchangeInfo"); exhash=sha(exraw); ex=json.loads(exraw); exmap={x.get("symbol"):x for x in ex.get("symbols",[])}
    ...
    for i,s in enumerate(sorted(EXPECTED),1):
        ...
        r={"symbol":s,"coin_id":coin_id,"provider":"BINANCE_SPOT","transport_host":"data-api.binance.vision","pair":pair,"qualification":"QUALIFICATION_ONLY","source_admitted":False,"scientific_credit":False,"prospective_credit":False,"d0_credit":0}
        try:
            bridge,bh,seen=cg_bridge(coin_id,s)
            ident=exmap.get(pair) or {}; ident_ok=ident.get("baseAsset")==s and ident.get("quoteAsset")=="USDT"
            raw=req(u(BINANCE_PUBLIC_DATA+"/api/v3/klines",{"symbol":pair,"interval":"1d","startTime":start_ms,"endTime":end_ms,"limit":1000})); arr=json.loads(raw)
