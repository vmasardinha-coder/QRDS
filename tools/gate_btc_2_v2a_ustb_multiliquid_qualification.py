...
def acct_keys(tx):
    ...
    meta=(tx or {}).get('meta') or {}; loaded=meta.get('loadedAddresses') or {}
...
def extract_trade(sig,tx):
    ...
    candidates=[]; common=set(pre)|set(post); by_owner=defaultdict(dict)
    for key in common:
        idx,mint,owner=key
    ...
    candidates.sort(key=lambda x:(-(x[1]*x[2]),x[0])); owner,qty,usd,px=candidates[0]
...
def main():
    ...
    try:
        for page in range(8):
            ...
            if oldest is not None and datetime.fromtimestamp(int(oldest),timezone.utc).date()<START: history_complete=True; break
        ...
        for row in sigs:
            ...
            seen.add(sig); raw,tx=rpc('getTransaction',[sig,{'encoding':'jsonParsed','maxSupportedTransactionVersion':0}])
            tx_hashes.append({'signature':sig,'sha256':hashlib.sha256(raw).hexdigest()}); t=extract_trade(sig,tx)
            if t: trades.append(t)
            time.sleep(0.12)
        trades.sort(key=lambda x:x['timestamp']); (out/'TRADES.json').write_text(json.dumps(trades,indent=2,sort_keys=True)+'\n')
