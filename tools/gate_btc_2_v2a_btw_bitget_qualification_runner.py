#!/usr/bin/env python3
"""Physical qualification-only capture for preregistered BTW/Bitget V2A source."""
...
PAIR="BTWUSDT"; PROVIDER="Bitget"; GRANULARITY="1Dutc"; LIMIT=200
...
def main()->int:
    p=argparse.ArgumentParser(); p.add_argument("--end",required=True); p.add_argument("--output",default="artifacts/gate_btc_2/v2a_btw_bitget_qualification"); p.add_argument("--max-pages",type=int,default=20); args=p.parse_args()
    ...
    try:
        ...
        if not rows: raise ValueError("no BTWUSDT historical candles returned")
