...
def call(method,params,retries=5):
 ...
 for n in range(retries):
  ...
  except Exception as e: last=e; time.sleep(min(8,2**n))
 raise RuntimeError(f'RPC_FAILED:{method}:{last}')
def block(n):
 _,x=call('eth_getBlockByNumber',[hex(n),False]); return x
def block_for(ts):
 _,latest=call('eth_blockNumber',[]); lo,hi=1,int(latest,16)
 while lo<hi:
  mid=(lo+hi)//2; b=block(mid); bt=int(b['timestamp'],16)
  if bt<ts: lo=mid+1
  else: hi=mid
 return lo
...
def main():
 ...
 error=None; candidates=[]; fills=[]; raw_hashes=[]
 try:
  s_ts=int(datetime.combine(START,datetime.min.time(),tzinfo=timezone.utc).timestamp()); e_ts=int(datetime.combine(END+timedelta(days=1),datetime.min.time(),tzinfo=timezone.utc).timestamp())-1
  sb=block_for(s_ts); eb=block_for(e_ts)
  step=50000; logs=[]
  for left in range(sb,eb+1,step):
   right=min(eb,left+step-1); raw,res=call('eth_getLogs',[{'fromBlock':hex(left),'toBlock':hex(right),'address':BUIDL,'topics':[TRANSFER]}]);raw_hashes.append(hashlib.sha256(raw).hexdigest());logs.extend(res or [])
  txhs=sorted({str(x.get('transactionHash')) for x in logs if x.get('transactionHash')})
  for h in txhs:
   ...
   trs=[decode_transfer(l) for l in rec.get('logs') or [] if str(l.get('topics',[None])[0] if l.get('topics') else '').lower()==TRANSFER]
   trs=[x for x in trs if x and x['token'] in {BUIDL,USDC}]
   b=[x for x in trs if x['token']==BUIDL and x['amount']>0]; u=[x for x in trs if x['token']==USDC and x['amount']>0]
   candidates.append({'tx_hash':h,'buidl_transfers':len(b),'usdc_transfers':len(u),'reactor_link':True})
   if len(b)!=1 or len(u)!=1: continue
   bn=int(rec['blockNumber'],16); bl=block(bn); ts=int(bl['timestamp'],16); d=datetime.fromtimestamp(ts,timezone.utc).date()
   if not START<=d<=END: continue
   bamt=b[0]['amount']/1e6; uamt=u[0]['amount']/1e6
   if bamt<=0 or uamt<=0: continue
   fills.append({'tx_hash':h,'timestamp':ts,'day':d.isoformat(),'buidl':bamt,'usdc':uamt,'price_usdc_per_buidl':uamt/bamt})
  ...
  qa=bool(fills) and len(daily)==33 and not missing and all(x['low']<=min(x['open'],x['close'])<=max(x['open'],x['close'])<=x['high'] for x in daily)
