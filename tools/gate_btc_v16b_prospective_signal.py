...
def build(panel_path: Path, historical_shortability_path: Path, signal_date: pd.Timestamp,
          snapshot_id: str, snapshot_available_at_utc: str, code_commit: str) -> dict:
    ...
    base = {
        ...
        "input_data_hashes": {"panel": panel_sha, "historical_shortability": short_sha},
        "universe_snapshot_id": snapshot_id,
        "universe_snapshot_available_at_utc": snapshot_available_at_utc,
        ...
        "source_coverage": {"panel": "OK", "historical_shortability": "STRICTLY_PRIOR_ONLY", "friday_shortability_used": False},
        "blocker_reason": None,
    }
    if len(g) < 20:
        ...
        base["model_state_hash"] = hobj({"model_hash": model_hash, "panel_sha": panel_sha, "signal_date": base["signal_date_utc"], "status": "BLOCKED"})
        return base
    ...
    base["model_state_hash"] = hobj({"model_hash": model_hash, "panel_sha": panel_sha, "historical_shortability_sha": short_sha, "signal_date": base["signal_date_utc"], "eligible_scores": scores})
    return base
...
def main() -> int:
    ...
    p.add_argument("--signal-date", required=True); p.add_argument("--universe-snapshot-id", required=True)
    p.add_argument("--universe-snapshot-available-at-utc", required=True); p.add_argument("--code-commit", required=True)
    p.add_argument("--output", required=True)
    a = p.parse_args()
    result = build(Path(a.panel), Path(a.historical_shortability), pd.Timestamp(a.signal_date), a.universe_snapshot_id, a.universe_snapshot_available_at_utc, a.code_commit)
