#!/usr/bin/env python3
"""Physical qualification-only capture for preregistered AERO/Coinbase V2A source."""
...
def parse_product(raw: bytes) -> dict:
    ...
    if obj.get("base_currency") != "AERO" or obj.get("quote_currency") != "USD":
...
def main() -> int:
    ...
    parser.add_argument("--output", default="artifacts/gate_btc_2/v2a_aero_coinbase_qualification")
    ...
    try:
        ...
        if not rows:
            raise ValueError("no AERO-USD historical candles returned")
    ...
    summary = {
        "schema_version": "GATE_BTC_2_V2A_AERO_COINBASE_QUALIFICATION_V1",
        "issue": 111,
        "symbol": "AERO",
