#!/usr/bin/env python3
"""Physical qualification-only capture for preregistered BDX/MEXC V2A source."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default="artifacts/gate_btc_2/v2a_bdx_mexc_qualification",
    )
    ...
    try:
        ...
        if not rows:
            raise ValueError("no BDXUSDT historical candles returned")
    ...
    summary = {
        "schema_version": "GATE_BTC_2_V2A_BDX_MEXC_QUALIFICATION_V1",
        "issue": 111,
        "symbol": "BDX",
        "coin_id": "beldex",
        "provider": PROVIDER,
