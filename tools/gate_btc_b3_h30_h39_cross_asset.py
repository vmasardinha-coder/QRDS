...
def load(a,p,tf):
 u=f'https://raw.githubusercontent.com/wesleyzilva/tradetech/main/CandlesHistoryDatas/{p}/{a}FUT_F_0_{tf}min.csv';r=requests.get(u,timeout=180);r.raise_for_status();x=pd.read_csv(StringIO(r.text),sep=';',dtype=str);c={z.lower().strip():z for z in x.columns};d=pd.DataFrame();d['timestamp']=pd.to_datetime(x[c['data']].str.strip()+' '+x[c['hora']].str.strip(),dayfirst=True)
