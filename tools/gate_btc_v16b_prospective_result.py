...
The builder does not accept user-supplied PnL. It recomputes all 20 asset PnLs,
actual short-funding contribution, frozen turnover cost and BTC benchmark from a
sealed ENTRY event plus verified evidence artifacts.
"""
...
def _verify_raw_record(record: dict, evidence_dir: Path, expected_day: str, expected_instrument: str) -> None:
    required = {"venue", "instrument", "date_utc", "close", "raw_file", "checksum_file", "raw_sha256", "expected_checksum_sha256", "source_url"}
    ...
    checksum_token = checksum_path.read_text(encoding="utf-8").strip().split()[0] if checksum_path.read_text(encoding="utf-8").strip() else ""
    if checksum_token.lower() != actual:
        raise ValueError("preserved Binance CHECKSUM does not match raw archive")
...
def _load_funding(csv_path: Path, summary_path: Path, entry_event: dict, exit_date: str) -> tuple[dict[str, float], str, dict]:
    ...
    evidence_hash = hobj({"funding_csv_sha256": csv_sha, "funding_summary_sha256": summary_sha})
    return by_asset, evidence_hash, {"csv_sha256": csv_sha, "summary_sha256": summary_sha, "source": summary.get("source"), "method": summary.get("method")}
...
def build(entry_event_path: Path, price_evidence_path: Path, funding_csv_path: Path, funding_summary_path: Path) -> dict:
    ...
    return {
        ...
        "source_coverage": {
            ...
            "price_archives": "BINANCE_DATA_VISION_CHECKSUM_VERIFIED",
...
def main() -> int:
    ...
    print(json.dumps({"status": result["status"], "gross_long_pnl": result["gross_long_pnl"], "gross_short_pnl": result["gross_short_pnl"], "short_funding_total": sum(result["short_funding"].values()), "transaction_cost": result["transaction_cost"], "net_pnl": result["net_pnl"], "ORDERS": 0, "REAL_CAPITAL": 0}, sort_keys=True))
    return 0
