...
def brnum(s):
    return pd.to_numeric(s.astype(str).str.replace(".", "", regex=False).str.replace(",", ".", regex=False), errors="raise")
...
def merge_agg(store, rows):
    for r in rows.itertuples(index=False):
        ...
        if cur is None:
            ...
            cur["high"] = max(cur["high"], float(r.high)); cur["low"] = min(cur["low"], float(r.low))
...
def aggregate_chunk(x, size):
    if x.empty:
        return pd.DataFrame()
    x = x.copy(); x["bucket_min"] = bucket_min_from_ms(x["time_ms"], size)
    x = x.sort_values(["root", "bucket_min", "time_ms", "trade_id"], kind="mergesort")
    g = x.groupby(["root", "bucket_min"], sort=False)
    return g.agg(
        high=("price", "max"), low=("price", "min"), volume=("qty", "sum"), trades=("trade_id", "size"),
        first_time=("time_ms", "first"), first_id=("trade_id", "first"), last_time=("time_ms", "last"),
        last_id=("trade_id", "last"), open=("price", "first"), close=("price", "last"),
    ).reset_index()


def rows_from_store(store, date, front):
    rows = []
    for (root, b), v in sorted(store.items(), key=lambda kv: (kv[0][0], kv[0][1])):
        hh = b // 60; mm = b % 60
        ts = pd.Timestamp(f"{date} {hh:02d}:{mm:02d}", tz="America/Sao_Paulo")
        rows.append({"timestamp": ts.isoformat(), "date": date, "root": root, "symbol": front[root],
                     "open": v["open"], "high": v["high"], "low": v["low"], "close": v["close"],
                     "volume": v["volume"], "trades": v["trades"]})
    return pd.DataFrame(rows)


def process_zip(raw: bytes, date: str, front: dict[str, str]):
    z = zipfile.ZipFile(io.BytesIO(raw))
    files = [x for x in z.infolist() if not x.is_dir()]
    if len(files) != 1:
        z.close(); raise RuntimeError(f"ZIP member count {len(files)}")
    member = files[0]
    if "_NEGOCIOSAVISTA.TXT" not in member.filename.upper():
        z.close(); raise RuntimeError(f"unexpected member {member.filename}")
    s1 = {}; s5 = {}
    stats = {"source_rows": 0, "front_rows": 0, "kept_rows": 0, "cancel_rows": 0, "unknown_actions": [], "off_lattice": 0}
    with z.open(member) as fh:
        for ch in pd.read_csv(fh, sep=";", encoding="iso-8859-1", usecols=USECOLS, dtype=str, chunksize=CHUNK, low_memory=False):
            ...
            if x.empty:
                continue
            x["price"] = brnum(x["PrecoNegocio"]); x["qty"] = brnum(x["QuantidadeNegociada"])
            x["trade_id"] = x["CodigoIdentificadorNegocio"].astype(str).str.strip(); x["time_ms"] = time_ms(x["HoraFechamento"])
            mins = (x["time_ms"] // 60000).astype(int)
            x = x[(mins >= 9 * 60) & (mins < 17 * 60 + 30)].copy()
            if x.empty:
                continue
            ztick = np.where(x["root"].eq("WIN"), 5.0, 0.5); ratio = x["price"].to_numpy() / ztick
            ok = np.abs(ratio - np.round(ratio)) <= 1e-7 * np.maximum(1.0, np.abs(ratio))
            stats["off_lattice"] += int((~ok).sum()); x = x.loc[ok].copy(); stats["kept_rows"] += len(x)
            merge_agg(s1, aggregate_chunk(x, 1)); merge_agg(s5, aggregate_chunk(x, 5))
    z.close()
    ...
    return rows_from_store(s1, date, front), rows_from_store(s5, date, front), stats, member.filename


def front_for(date: str):
    # Frozen August 2026 front contracts used by the H1 protocol before roll.
    return {"WIN": "WINQ26", "WDO": "WDOU26"}


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--probe-dir", default="artifacts/b3_h1_source_probe")
    a = ap.parse_args()
    root = Path(a.probe_dir)
    rows = []
    for day in sorted(p for p in root.iterdir() if p.is_dir()):
        date = day.name
        files = sorted(day.glob(f"{date}_legacy_NEGOCIOSAVISTA.zip"))
        if not files:
            rows.append({"date": date, "qualified": False, "error": "SOURCE_ZIP_MISSING"}); continue
        raw = files[0].read_bytes(); front = front_for(date)
        try:
            m1, m5, stats, member = process_zip(raw, date, front)
            cov = {r: int(len(m5[m5.root == r])) for r in ["WIN", "WDO"]}
            qualified = cov["WIN"] == 102 and cov["WDO"] == 102
            m1.to_csv(day / "M1.csv.gz", index=False, compression="gzip")
            m5.to_csv(day / "M5.csv.gz", index=False, compression="gzip")
            row = {"date": date, "WIN": front["WIN"], "WDO": front["WDO"], "member": member,
                   "coverage_m5": cov, "m1_rows": int(len(m1)), "m5_rows": int(len(m5)), "stats": stats,
                   "qualified": qualified, "sha256": sha(raw), "zip_bytes": len(raw)}
        except Exception as e:
            row = {"date": date, "WIN": front["WIN"], "WDO": front["WDO"], "qualified": False, "error": repr(e)}
        dump(day / "H1_STRUCTURAL_QA.json", row); rows.append(row)
    passed = [r["date"] for r in rows if r.get("qualified")]
    summary = {"schema": "gate_btc.b3.h1.structural_qa.v1", "research_only": True, "shadow_only": True,
               "orders": 0, "real_capital": 0, "economics_locked": True, "rows": rows,
               "dates_structural_pass": passed, "h1_increment_candidate": len(passed)}
    dump(root / "H1_STRUCTURAL_QA_SUMMARY.json", summary)
    print(json.dumps(summary, indent=2, ensure_ascii=False))
    return 0 if len(passed) == len(rows) and rows else 2
