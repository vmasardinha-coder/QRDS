...
def main() -> int:
    ...
    args = p.parse_args()
    if date.fromisoformat(args.cutoff) < FREEZE:
    ...
    if target.exists():
        ...
        try:
            ...
            diagnostic = {
                ...
                "detail": str(exc), "ledger_mutated": False,
        ...
        identity_diag.write_text(json.dumps({
            ...
            "cutoff": args.cutoff, **resolution, "ledger_mutated": False,
        }, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        print(json.dumps({"cutoff": args.cutoff, **resolution,
                          "m1_n": len(existing["m1"]["rows"]),
                          "m2_n": len(existing["m2"]["rows"])}, sort_keys=True))
        return 0
    try:
        assert_new_cutoff_monotonic(args.ledger_dir, args.cutoff)
    except MomentumIdentityConflict as exc:
    ...
    print(json.dumps({"cutoff": args.cutoff, "status": "PASS_PROSPECTIVE_SHADOW",
                      "snapshot_sha256": candidate["snapshot_sha256"],
                      "m1_n": len(candidate["m1"]["rows"]),
                      "m2_n": len(candidate["m2"]["rows"])}, sort_keys=True))
    return 0
