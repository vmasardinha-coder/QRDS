#!/usr/bin/env python3
"""Physical qualification-only capture for preregistered AKE/MEXC V2A source."""
from __future__ import annotations
import argparse, hashlib, json, time, urllib.parse, urllib.request
from datetime import date, datetime, timedelta, timezone
from pathlib import Path
API="https://api.mexc.com"; PAIR="AKEUSDT"; PROVIDER="MEXC"; LIMIT=1000
...
def parse_info(raw):
    ...
    if len(hits)!=1 or hits[0].get("baseAsset")!="AKE" or hits[0].get("quoteAsset")!="USDT": raise ValueError("instrument identity mismatch")
    return hits[0]
def parse(raw):
...
def main():
    ...
    try:
        ...
        if not rows:raise ValueError("no in-window AKEUSDT historical candles returned")
