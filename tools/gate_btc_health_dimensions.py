...
This sidecar is reporting-only. It never turns stale, missing, failed or blocked
evidence into GREEN and never grants operational approval.
"""
...
def classify_component(name: str, comp: dict[str, Any]) -> dict[str, Any]:
    ...
    hint = str(comp.get("collection_health_hint", ""))
    diagnostic = f"{status} {comp.get('last_run_state', '')}".upper()
    ...
    if freshness.startswith("STALE"):
    ...
    elif hint.startswith("RED"):
        collection = hint
        science = "PRESERVED_DELIVERY_FAILURE"
    elif hint.startswith("AMBER"):
        collection = hint
        science = "DEPENDENCY_BLOCKED_NOT_YET_EVALUABLE"
    elif status.upper() in {"MISSING", "FAIL", "FAILED", "ERROR"} or "FAILED" in diagnostic or diagnostic.startswith("ERROR"):
        collection = "RED_FAILED_DELIVERY"
        science = "INSUFFICIENT_EVIDENCE"
    elif status.upper().startswith("OPEN_DIAGNOSTIC"):
        collection = "RED_DIAGNOSTIC_OPEN"
        science = "PRESERVED_DELIVERY_FAILURE"
    elif "BLOCKED" in diagnostic:
        collection = "AMBER_BLOCKED"
        science = "DEPENDENCY_BLOCKED_NOT_YET_EVALUABLE"
        ...
        science = "ACTIVE_SHADOW_EVIDENCE"
    return {
...
def build(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if red or (not amber and report.get("delivery_complete") is not True):
        collection_delivery = "RED_INCOMPLETE_DELIVERY"
    elif amber:
        collection_delivery = "AMBER_INCOMPLETE_DELIVERY"
    else:
        collection_delivery = "GREEN"
    payload = {
        ...
        "interpretation": "A collection/delivery defect is component-scoped and does not retroactively invalidate previously admitted scientific evidence. No stale, missing, failed or blocked component is promoted to GREEN.",
    }
...
def main() -> int:
    ...
    parser.add_argument("--reporting-state", type=Path, required=True)
