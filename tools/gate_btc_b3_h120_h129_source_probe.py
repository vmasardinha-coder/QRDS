...
def scan(raw):
 f={x:0 for x in FIELDS};p={x:0 for x in PREFIXES};samples=[]
...
def main():
 ...
 for d in DATES:
  ...
  try:
   n,x,inner=xml_from(r.content);fc,pc,sm=scan(x);rec.update({'xml_name':n,'xml_sha256':hashlib.sha256(x).hexdigest(),'nested_zip_sha256':inner,'field_counts':fc,'prefix_counts':pc,'sample_tickers':sm})
   needed=fc['TckrSymb']>0 and fc['TradQty']>0 and fc['OpnIntrst']>0 and fc['MinPric']>0 and fc['MaxPric']>0 and (fc['RglrTraddCtrcts']>0 or fc['FinInstrmQty']>0) and all(pc[k]>0 for k in PREFIXES)
