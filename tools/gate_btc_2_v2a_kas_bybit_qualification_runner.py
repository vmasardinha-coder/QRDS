#!/usr/bin/env python3
"""Physical qualification-only capture for preregistered KAS/Bybit V2A source.
...
BASE = "https://public.bybit.com/spot/KASUSDT/"
PAIR = "KASUSDT"
PROVIDER = "Bybit"
DAY_RE = re.compile(r'href=["\'](KASUSDT_(\d{4}-\d{2}-\d{2})\.csv\.gz)["\']')
...
def main() -> int:
    ...
    p.add_argument("--output", default="artifacts/gate_btc_2/v2a_kas_bybit_qualification")
    ...
    try:
        ...
        if not entries:
            raise ValueError("no exact KASUSDT daily archive objects")
    ...
    summary = {
        "schema_version": "GATE_BTC_2_V2A_KAS_BYBIT_QUALIFICATION_V1_ARCHIVE",
        "issue": 111,
        "symbol": "KAS",
        "coin_id": "kaspa",
        "provider": PROVIDER,
