#!/usr/bin/env python3
"""Physical qualification for preregistered XMR/USDT Gate spot route.
...
PAIR="XMR_USDT"; COIN_ID="monero"
GATE="https://api.gateio.ws/api/v4"; UA="QRDS-GateBTC2-ResearchOnly/xmr-gate-v1"
...
def qa(rows):
    ...
    return {
        'daily_bucket_count':len(rows),'missing_days':miss,'duplicate_days':len(days)-len(have),
        'monotonic_daily_dates':days==sorted(days),'finite_ohlc_and_invariant':numeric,'qa_pass':ok
    }
...
def main():
    ...
    result={
        'schema_version':'GATE_BTC_2_V2A_XMR_GATE_EXACT_SOURCE_QUALIFICATION_V1',
        'symbol':'XMR','coin_id':COIN_ID,'provider':'GATE_SPOT','pair':PAIR,
    ...
    try:
        cg_raw=req(url(f"https://api.coingecko.com/api/v3/coins/{COIN_ID}/tickers",{'exchange_ids':'gate','order':'trust_score_desc','page':'1'}))
        cg=json.loads(cg_raw); tick=cg.get('tickers',[]) if isinstance(cg,dict) else []
        bridge=any(str((t.get('market') or {}).get('identifier','')).lower()=='gate' and str(t.get('base','')).upper()=='XMR' and str(t.get('target','')).upper()=='USDT' for t in tick)

        inst_raw=req(GATE+'/spot/currency_pairs/'+PAIR); inst=json.loads(inst_raw)
        ident_ok=isinstance(inst,dict) and str(inst.get('id','')).upper()==PAIR and str(inst.get('base','')).upper()=='XMR' and str(inst.get('quote','')).upper()=='USDT'
        ...
        arr=json.loads(candle_raw)
        rows=[]
        if isinstance(arr,list):
            for x in arr:
                if not isinstance(x,list) or len(x)<6: continue
                day=datetime.fromtimestamp(int(float(x[0])),timezone.utc).date().isoformat()
                rows.append({'day':day,'open':float(x[5]),'high':float(x[3]),'low':float(x[4]),'close':float(x[2])})
        q=qa(rows)
        result.update({
            'coingecko_exact_market_bridge':bridge,'bridge_response_sha256':sha(cg_raw),
            'official_identity_ok':ident_ok,'instrument_response_sha256':sha(inst_raw),
            'candle_response_sha256':sha(candle_raw),**q
        })
