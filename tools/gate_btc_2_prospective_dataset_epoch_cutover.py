...
Orchestration/data-readiness only: never repairs history, grants economics, changes
methodology, or assigns a retroactive D0.
"""
...
def assess(status_path: Path, snapshots_dir: Path, registry_path: Path) -> dict[str, Any]:
    ...
    survivorship = bool(snapshot.get("survivorship_bias_present", status.get("survivorship_bias_present", True)))
    ...
    if not isinstance(snapshot.get("universe_archive"), dict) or not snapshot["universe_archive"].get("archive_sha256"):
        blockers.append("IMMUTABLE_UNIVERSE_CAPTURE_MISSING")
    if not isinstance(snapshot.get("quality_archive"), dict) or not snapshot["quality_archive"].get("archive_sha256"):
        blockers.append("IMMUTABLE_QUALITY_CAPTURE_MISSING")
    ...
    if not registry_path.exists():
        ...
        for entry in entries:
            ...
            if not entry.get("provenance_sha256"):
                ...
                break
            if not entry.get("timezone") or not entry.get("cutoff_semantics"):
                blockers.append("SOURCE_TIME_CUTOFF_CONTRACT_INCOMPLETE")
                break
            if entry.get("qa_pass") is not True:
                blockers.append("SOURCE_QA_NOT_PROVEN")
                break
            if entry.get("observed_vs_derived") != "OBSERVED":
                blockers.append("SOURCE_OBSERVED_LABEL_NOT_PROVEN")
                break
    ...
    return {
        ...
        "quality_sha256": source_hashes.get("quality_sha256"),
        "capture_archives": {
            "universe": snapshot.get("universe_archive"),
            "quality": snapshot.get("quality_archive"),
            "failures": snapshot.get("failures_archive"),
        },
        ...
        "per_symbol_source_contracts": entries if eligible else [],
        "blockers": blockers,
        "historical_credit": 0,
...
def write_d0_if_eligible(assessment: dict[str, Any], d0_path: Path) -> bool:
    ...
    if d0_path.exists():
        ...
        required = {"schema": D0_SCHEMA, "epoch_id": EPOCH_ID, "preregistration_commit_sha": PREREG_COMMIT}
    ...
    record = {
        ...
        "quality_sha256": assessment["quality_sha256"],
        "capture_archives": assessment["capture_archives"],
        ...
        "per_symbol_source_contracts": assessment["per_symbol_source_contracts"],
        "schema_timezone_cutoff_pit_qa": "PASS_BY_CUTOVER_GATE",
