...
if [ ! -f "$MOD" ]; then
  echo "[QRDS 30B HOTFIX] Missing Phase 30 module: $MOD"
  exit 1
fi

echo "[QRDS 30B HOTFIX] Applying robust readiness alias override..."
cp "$MOD" "$MOD.bak_30B_alias_override"
...
marker = "\n# --- QRDS HOTFIX 30B READINESS ALIAS OVERRIDE START ---\n"
if marker in text:
    text = text.split(marker)[0].rstrip() + "\n"

override = """
# --- QRDS HOTFIX 30B READINESS ALIAS OVERRIDE START ---
# Robust read-only compatibility layer for previous phase readiness keys.
# This intentionally does NOT unlock edge, shadow decision, safe-apply, promotion,
# allocation, recommendations, or operational decisions.

def _qrds30b_ready_from_aliases(p: dict[str, Any], aliases: list[str]) -> tuple[bool, str]:
    for key in aliases:
...
def _component_rows(phases: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for station, component_id, phase_key, aliases, label in specs:
        ...
        ready, resolved_key = _qrds30b_ready_from_aliases(p, aliases)
        rows.append({
            "station": station,
...
# --- QRDS HOTFIX 30B READINESS ALIAS OVERRIDE END ---
"""

path.write_text(text.rstrip() + "\n" + override + "\n", encoding="utf-8")
PY

echo "[QRDS 30B HOTFIX] Adding focused regression test..."
cat > "$PROJECT/tests/regression/test_phase30_readiness_alias_override_hotfix.py" <<'PY'
import json
from pathlib import Path
...
echo "[QRDS 30B HOTFIX] Running targeted tests..."
cd "$PROJECT"
pytest -q \
...
echo "[QRDS 30B HOTFIX] Running full test suite..."
pytest -q tests/safety tests/unit tests/integration tests/regression tests/docs

echo "[QRDS 30B HOTFIX] Regenerating Phase 30 report..."
cd "$ROOT"
...
print("[QRDS 30B HOTFIX] Component readiness:")
...
echo "[QRDS 30B HOTFIX] Committing changes..."
git add -A
git commit -m "Hotfix Phase 30 dashboard readiness alias override" || true
git push || true

echo "[QRDS 30B HOTFIX] Final status:"
git status --short
