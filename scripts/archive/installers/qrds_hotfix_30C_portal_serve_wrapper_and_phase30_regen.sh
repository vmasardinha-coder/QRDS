...
echo "[QRDS 30C HOTFIX] Patching qrds_portal_serve.sh to be test-safe and Codespaces-safe..."
if [ -f "$ROOT/qrds_portal_serve.sh" ]; then
  cp "$ROOT/qrds_portal_serve.sh" "$ROOT/qrds_portal_serve.sh.bak_30C"
fi
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --port)
      ...
      ;;
    --bind)
      BIND_HOST="${2:?missing value for --bind}"
      shift 2
      ;;
    --no-build|--skip-build)
      BUILD_PORTAL="0"
...
EOF
      ...
      ;;
    *)
      echo "[QRDS PORTAL SERVE] Unknown argument: $1" >&2
      exit 2
      ;;
  esac
done
...
echo "[QRDS 30C HOTFIX] Adding regression test for robust portal serve wrapper..."
cat > "$PROJECT/tests/regression/test_portal_serve_wrapper_30c_hotfix.py" <<'PY'
...
echo "[QRDS 30C HOTFIX] Running portal serve targeted tests..."
cd "$PROJECT"
pytest -q \
...
echo "[QRDS 30C HOTFIX] Running Phase 30 targeted tests..."
pytest -q \
...
echo "[QRDS 30C HOTFIX] Running full test suite..."
pytest -q tests/safety tests/unit tests/integration tests/regression tests/docs

echo "[QRDS 30C HOTFIX] Regenerating Phase 30 report..."
cd "$ROOT"
...
print("[QRDS 30C HOTFIX] Component readiness:")
...
echo "[QRDS 30C HOTFIX] Committing changes..."
git add -A
git commit -m "Hotfix portal serve wrapper and regenerate Phase 30" || true
git push || true

echo "[QRDS 30C HOTFIX] Final status:"
git status --short
