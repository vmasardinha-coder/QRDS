...
OPSEC_OUTPUT_DIR="artifacts/operational_security"
...
SECURITY_STATE="${QRDS_SECURITY_STATE:-UNDER_REVIEW}"
SECRETS_SCAN_STATE="${QRDS_SECRETS_SCAN_STATE:-PASS}"
RUN_STACK=1
RUN_RISK=1

usage() {
  cat <<'USAGE'
QRDS Operational Security from Evidence Stack Serve

Generates the 8L → 8R evidence stack first, generates 8U Risk Model from that
stack, then runs the 8V Operational Security Review Gate using all generated
reports. Research-only; cannot unlock operational use.
...
Options:
  ...
  --output-dir DIR              Operational-security output. Default: artifacts/operational_security
  ...
  --security-state STATE        Default: UNDER_REVIEW
  --secrets-scan-state STATE    Default: PASS
  --skip-stack-refresh          Do not regenerate 8L → 8R; only read existing stack artifacts.
  --skip-risk-refresh           Do not regenerate 8U; only read existing risk artifact.
  -h, --help                    Show help.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --output-dir) OPSEC_OUTPUT_DIR="${2:?missing value for --output-dir}"; shift 2 ;;
    --output-dir=*) OPSEC_OUTPUT_DIR="${1#--output-dir=}"; shift ;;
    ...
    --security-state) SECURITY_STATE="${2:?missing value for --security-state}"; shift 2 ;;
    --security-state=*) SECURITY_STATE="${1#--security-state=}"; shift ;;
    --secrets-scan-state) SECRETS_SCAN_STATE="${2:?missing value for --secrets-scan-state}"; shift 2 ;;
    --secrets-scan-state=*) SECRETS_SCAN_STATE="${1#--secrets-scan-state=}"; shift ;;
    --skip-stack-refresh) RUN_STACK=0; shift ;;
    --skip-risk-refresh) RUN_RISK=0; shift ;;
    -h|--help) usage; exit 0 ;;
...
if [[ ! -d "$PROJECT_DIR" ]]; then
  echo "[QRDS 8V] ERROR: crypto_decision_lab project not found under $ROOT_DIR" >&2
  exit 1
fi

if [[ "$RUN_STACK" == "1" ]]; then
  if [[ ! -x "$ROOT_DIR/qrds_evidence_stack.sh" ]]; then
    echo "[QRDS 8V] ERROR: qrds_evidence_stack.sh not found/executable. Install Sprint 8S first." >&2
    exit 1
  fi
  echo "[QRDS 8V] Refreshing upstream evidence stack 8L -> 8R..."
  ...
  echo "[QRDS 8V] Skipping upstream stack refresh by request."
fi
...
report_path() {
  ...
  if [[ "$STACK_OUTPUT_DIR" = /* ]]; then
    ...
    # qrds_operational_security.sh executes from crypto_decision_lab, so
    # relative report paths must be relative to crypto_decision_lab.
    printf '%s/%s' "$STACK_OUTPUT_DIR" "$rel"
  fi
}
...
for report in "${EVIDENCE_REPORTS[@]}"; do
  ...
  if [[ -f "$check_path" ]]; then
    EXISTING_EVIDENCE+=("$report")
  else
    echo "[QRDS 8V] WARN: missing upstream evidence report: $check_path" >&2
  fi
done
...
if [[ "$RUN_RISK" == "1" ]]; then
  if [[ ! -x "$ROOT_DIR/qrds_risk_model.sh" ]]; then
    echo "[QRDS 8V] ERROR: qrds_risk_model.sh not found/executable. Install Sprint 8U first." >&2
    exit 1
  fi
  echo "[QRDS 8V] Generating 8U risk model from refreshed stack..."
  bash "$ROOT_DIR/qrds_risk_model.sh" \
    --output-dir "$STACK_OUTPUT_DIR/risk_model" \
    --symbols "$SYMBOLS" \
  ...
  echo "[QRDS 8V] Skipping risk-model refresh by request."
fi

RISK_REPORT="$(report_path 'risk_model/risk_model_gate.json')"
REPORT_CANDIDATES=(
  "${EVIDENCE_REPORTS[@]}"
  "$RISK_REPORT"
...
for report in "${REPORT_CANDIDATES[@]}"; do
  ...
  if [[ -f "$check_path" ]]; then
    ...
    echo "[QRDS 8V] WARN: missing report for operational-security packet: $check_path" >&2
  fi
done
REPORTS_CSV="$(IFS=,; echo "${EXISTING[*]}")"

cat <<EOF2
[QRDS 8V] Upstream reports found: ${#EXISTING[@]}/8
[QRDS 8V] Stack output dir: $STACK_OUTPUT_DIR
[QRDS 8V] Operational-security output dir: $OPSEC_OUTPUT_DIR
[QRDS 8V] Symbols: $SYMBOLS
[QRDS 8V] Scope: research-only; no API key, no account, no signal, no recommendation, no order.
EOF2

exec bash "$ROOT_DIR/qrds_operational_security_serve.sh" \
  --output-dir "$OPSEC_OUTPUT_DIR" \
  --symbols "$SYMBOLS" \
  --reports "$REPORTS_CSV" \
  --binance-mode SIMULATION_FIXTURE_REPLAY \
  --okx-mode PUBLIC_CACHE_OFFLINE \
  --bybit-mode BLOCKED_PENDING \
  --secrets-scan-state "$SECRETS_SCAN_STATE" \
