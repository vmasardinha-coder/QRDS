...
class CollectorSupervisorRegistryTests(unittest.TestCase):
    ...
    def test_registry_delivery_surfaces_are_explicit(self):
        ...
        self.assertEqual(c['V16B']['expected_workflow_job'], 'gate-btc-v16b-next-gate-watchdog.yml')
        self.assertIsNone(c['V16B']['expected_artifact'])
        self.assertEqual(c['V16B']['expected_ledger'], 'DISCOVER_RUNTIME')
        self.assertEqual(c['FACTORY_CURRENT_GENERATION']['expected_workflow_job'], 'gate-btc-b3-factory-continuation.yml')
        self.assertIsNone(c['FACTORY_CURRENT_GENERATION']['expected_artifact'])
        self.assertEqual(c['FACTORY_CURRENT_GENERATION']['expected_ledger'], 'runtime/autonomous_science/CURRENT.json')
