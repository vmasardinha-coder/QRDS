...
MODULE = Path(__file__).parents[1] / "tools" / "gate_btc_factory" / "b3_derivatives_type1_full_zip_inspector.py"
spec = importlib.util.spec_from_file_location("b3_type1_full", MODULE)
...
def make_zip(text: str) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("04-09-2026_NEGOCIOSAVISTA_DRV.txt", text.encode("latin-1"))
    return buf.getvalue()
...
class Resp:
    def __init__(self, raw: bytes, status: int = 200):
        ...
        self.headers = {"content-disposition": "attachment; filename=04-09-2026_NEGOCIOSAVISTA_DRV.zip", "content-type": "application/octet-stream"}
...
def test_type1_full_zip_detects_win_but_never_opens_gate():
    raw = make_zip("CodigoInstrumento;PrecoNegocio;QuantidadeNegociada;HoraFechamento\nWINV26;140000;1;101500000\nDI1F27;13;5;101501000\n")
    session = Session(raw)
    result = mod.inspect(session, "2026-09-04")
    assert session.urls == ["https://arquivos.b3.com.br/rapinegocios/tickercsv/2026-09-04?type=1"]
    assert result["exact_win_identity_observed_in_payload"] is True
    assert result["win_symbols"] == ["WINV26"]
    assert result["drv_filename_observed"] is True
...
def test_type1_zip_without_win_fails_closed():
    raw = make_zip("CodigoInstrumento;PrecoNegocio;QuantidadeNegociada;HoraFechamento\nDI1F27;13;5;101501000\n")
    result = mod.inspect(Session(raw), "2026-09-04")
    assert result["exact_win_identity_observed_in_payload"] is False
    assert result["status"] == "TYPE1_DRV_ZIP_HAS_NO_WIN_IDENTITY_FAIL_CLOSED"
    assert result["strict_source_gate_green"] is False


def test_type1_non_zip_fails_closed():
