...
DECISION_PATH = ROOT / "tools" / "gate_btc_2_checkpoint7_evidence_inventory_decision_v1.json"
WORKFLOW_PATH = ROOT / ".github" / "workflows" / "gate-btc-2-official-evidence-inventory.yml"
...
class GateBTC2Checkpoint7EvidenceInventoryDecisionTests(unittest.TestCase):
    ...
    def test_decision_hash_is_deterministic(self):
        ...
        computed = hashlib.sha256(json.dumps(unsigned, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")).hexdigest()
        self.assertEqual(claimed, computed)

    def test_bound_implementation_and_historical_workflow_evidence_remain_valid(self):
    ...
    def test_current_inventory_workflow_preserves_fail_closed_safety(self):
    ...
    def test_runtime_inventory_is_exact_but_has_no_admissible_candidate(self):
        ...
        self.assertEqual(inventory["status"], "BLOCKED_NO_ADMISSIBLE_OFFICIAL_DATASET_CANDIDATE")
        ...
        self.assertEqual(inventory["tabular_discovery"]["category_counts"]["unclassified"], 0)
    ...
    def test_status_claims_cannot_override_the_gate_or_unlock_economics(self):
        for claim in self.payload["deep_sweep"]["historical_qrds_claims"]:
            self.assertIn(claim["admission_effect"], {"STATUS_ONLY_CANNOT_REPLACE_PHYSICAL_DATASET","STATUS_ONLY_CANNOT_REPLACE_PHYSICAL_FUNDING_OR_OPEN_INTEREST_BYTES","SEPARATE_RESEARCH_DECISION_CANNOT_OVERRIDE_GATE_BTC_2_CONTRACT"})
    ...
    def test_recovery_order_preserves_no_backfill_and_parallel_isolation(self):
        ...
        self.assertEqual(self.payload["isolation"], {"b3_mutations":0,"delta_mutations":0,"economic_families_released":0,"incumbent_mutations":0,"regime_mutations":0,"runtime_mutations":0})
