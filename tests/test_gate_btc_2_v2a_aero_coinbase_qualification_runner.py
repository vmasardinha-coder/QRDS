...
from tools.gate_btc_2_v2a_aero_coinbase_qualification_runner import parse_candles, parse_product


class TestAEROCoinbaseQualificationRunner(unittest.TestCase):
    def test_parse_product_exact_pair(self):
        raw = json.dumps({"id": "AERO-USD", "base_currency": "AERO", "quote_currency": "USD"}).encode()
        product = parse_product(raw)
        self.assertEqual(product["id"], "AERO-USD")

    def test_parse_product_identity_mismatch_fails_closed(self):
        raw = json.dumps({"id": "AERO-USD", "base_currency": "OTHER", "quote_currency": "USD"}).encode()
