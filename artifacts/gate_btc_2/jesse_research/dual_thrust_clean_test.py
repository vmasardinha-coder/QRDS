...
def fetch(ex, symbol, total=2400):
    out=[]
...
def test(rows):
    ...
    while i < len(rows)-HOLD:
        ...
        if side:
            exitp=rows[i+HOLD][4]
            ret=side*(exitp/close-1)-ROUND_TRIP_COST
            trades.append(ret)
            i += HOLD
        else:
            i += 1
...
for s in SYMBOLS:
    rows=fetch(ex,s)
    results[s]={'bars':len(rows),'test':test(rows)}
report={'research_only':True,'shadow_only':True,'factory_modified':False,'source_system':'jesse','jesse_core_sha':'432cce8a4b91828ce34dcaffac3f8e67354d061c','example_source':'Dual Thrust','implementation_note':'Test uses a clean causal abstraction; upstream example contains a likely down_max_high column defect and is not executed verbatim.','frozen':{'up_len':UP_LEN,'down_len':DOWN_LEN,'up_coeff':UP_COEFF,'down_coeff':DOWN_COEFF,'hold_hours':HOLD,'round_trip_cost':ROUND_TRIP_COST},'results':results}
